...
module.exports={
	...
	activate: function(state) {
		...
		this.commandSubscription.add(atom.commands.add(
		    "atom-text-editor",
		    "rectangle-selection:select-to-beginning-Of-line",(event) => {
			this.commandSelection(() => {
		...
		this.commandSubscription.add(atom.commands.add(
		    "atom-text-editor",
		    "rectangle-selection:select-to-end-of-line",(event) => {
			this.commandSelection(() => {
				atom.workspace.getActiveTextEditor().
				    selectToEndOfLine();
			});
...
//					atom.notifications.addInfo("Rectangle undo");
			...
			this.undoSubscription.add(textEditor.onDidDestroy(() => {
				this.patchedTextEditors.splice(
				    this.patchedTextEditors.indexOf(textEditor),1);
			}));
		});
	},
	
	consumeStatusBar: function(statusBar) {
		for (var tile of statusBar.getLeftTiles())
		{
			if (tile.getItem().getAttribute!=null &&
			    tile.getItem().getAttribute("is")=="status-bar-selection")
			{
				this.statusBarSelection=tile.getItem();
				this.statusBarSelection.getActiveTextEditor=function() {
					...
					return {
						...
						getSelectedBufferRange : function() {
							...
							if (rectangleSelection!=null &&
							    rectangleSelection.mainModule.
							    isSelecting(textEditor))
							{
								return textEditor.getSelectedBufferRanges().
								    reduce((r1,r2) => r1.union(r2));
							}
							
							return textEditor.getSelectedBufferRange();
						},
					};
				};
			}
...
*/
	...
	adjustSelection: function(textEditor) {
		...
		if (this.statusBarSelection!=null)
		{
			this.statusBarSelection.updateCount();
			// selection の追加・削除を observe してないので
		}
	},
